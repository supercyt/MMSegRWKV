...
class Predictor:
    ...
    def predict_raw_probability(model_output, properties):
        ...
        d, w, h = shape_after_cropping_before_resample[0], shape_after_cropping_before_resample[1], \
        shape_after_cropping_before_resample[2]
        print(f"resample....")
        ...
        try:
            with torch.no_grad():
                ...
                for c in range(channel):
                    resample_output[c] = \
                    torch.nn.functional.interpolate(model_output[c][None, None], mode="trilinear", size=(d, w, h))[0, 0]
    ...
    def predict_noncrop_probability(model_output, properties):
        ...
        if len(model_output.shape) == 3:
            ...
            if isinstance(shape_before_cropping[0], torch.Tensor):
                shape_before_cropping = [shape_before_cropping[0].item(), shape_before_cropping[1].item(),
                                         shape_before_cropping[2].item()]

            none_crop_pred = np.zeros([shape_before_cropping[0], shape_before_cropping[1], shape_before_cropping[2]],
                                      dtype=np.uint8)
            ...
            none_crop_pred = np.zeros(
                [model_output.shape[0], shape_before_cropping[0], shape_before_cropping[1], shape_before_cropping[2]],
                dtype=np.uint8)
            bbox_used_for_cropping = properties["bbox_used_for_cropping"]
        ...
        with torch.no_grad():
            ...
            with torch.autocast("cuda", enabled=True) if device.type == "cuda" else dummy_context():
                ...
                if mirror_axes is not None:
                    ...
                    assert max(mirror_axes) <= len(
                        x.shape) - 3, 'mirror_axes does not match the dimension of the input!'
                    ...
                    if 0 in mirror_axes and 1 in mirror_axes and 2 in mirror_axes:
                        prediction += torch.flip(window_infer(torch.flip(x, (2, 3, 4)), model, **kwargs),
                                                 (2, 3, 4)).cpu()
                        torch.cuda.empty_cache()
